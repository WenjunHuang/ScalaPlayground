...
object GoogleChartBuilder {
  ...
  def dataSeries(allPercentiles: Seq[TreeMap[Int, Long]], meanValues: Seq[Double], sb: StringBuilder) = {
  ...
  def percentileLabels(percentiles: TreeMap[Int, Long], sb: StringBuilder) = {
    sb.append("chxl=1:|")
  ...
  def appendGridSpacing(maxValue: Long, sb: StringBuilder) = {
    sb.append("chg=0,10")
  }
  ...
  def appendLegend(statistics: Seq[Stats], sb: StringBuilder, legend: (Stats) => String) = {
  ...
  def barColors(numberOfSeries: Int, sb: StringBuilder) = {
    sb.append("chco=")
