...
import akkacluster.wordscount.JobMaster._
import akkacluster.wordscount.JobReceptionist.WordCount
...
class JobMaster extends Actor with ActorLogging with CreateWorkerRouter {
  ...
  def working(jobName: String, receptionist: ActorRef, cancellable: Cancellable): Receive = {
    ...
    case ReceiveTimeout =>
      if (workers.isEmpty) {
        log.info(s"No workers respond in time.Cancelling job $jobName.")
        stop(self)
  ...
  def createWorkerRouter: ActorRef = {
    context.actorOf(
      ClusterRouterPool(BroadcastPool(10), ClusterRouterPoolSettings(
        totalInstances = 1000, maxInstancesPerNode = 20,
        allowLocalRoutees = false, useRole = None)).props(Props[JobWorker]), name = "worker-router")
  }
}